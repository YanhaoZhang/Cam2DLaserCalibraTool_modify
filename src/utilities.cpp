...
// yanhao: debug save obs to check if they are same among different run
void saveobs(std::vector<Oberserve> obs,std::string saveobs_path){

//    std::string fn = saveobs_path + "obs.txt";
    std::ofstream foutC;
    foutC.open("/home/yanhao/LaserCameraCal_ws/data/obs.txt");

    if(!foutC.is_open()) {
        std::cerr << " can't open cam obs.txt "<<std::endl;
        return;
    }
...
//    Oberserve obk = obs[0];
//    std::vector<Eigen::Vector3d> calibra_pts = obk.points_on_line;
//    for (size_t i = 0; i < calibra_pts.size(); ++i) {
//        Eigen::Vector3d calibra_pts_i = calibra_pts[i];
//        foutC.setf(std::ios::fixed, std::ios::floatfield);
//        foutC.precision(3);
//
//        foutC << calibra_pts_i(0) << " "
//              << calibra_pts_i(1) << " "
//              << calibra_pts_i(2) <<std::endl;
//    }
//    foutC.close();
...
void getPointsIntensity(const sensor_msgs::LaserScan& scan_in,std::vector<float>& Intensity_oneScan){
    // http://docs.ros.org/melodic/api/sensor_msgs/html/msg/LaserScan.html
    size_t n_pts = scan_in.ranges.size ();

    for (size_t i = 0; i < n_pts; ++i) {
        Intensity_oneScan.push_back(scan_in.intensities[i]);
    }
}

float getAngleIncrement(const sensor_msgs::LaserScan& scan_in) {

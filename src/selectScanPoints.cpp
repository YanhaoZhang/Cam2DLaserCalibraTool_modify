...
#include <sensor_msgs/LaserScan.h>   //Yanhao: for use reflector
...
std::vector< Eigen::Vector3d > AutoGetLinePts(const std::vector<Eigen::Vector3d> points, float angle_increment, bool debug)
{
...
//        std::cout << points.at(id).transpose() <<" "<<points.at(id+1).transpose() <<std::endl;
    // 假设每个激光点之间的夹角为0.3deg, angle_increment = 0.25deg
    // step 1: 如果有激光标定板，那么激光标定板必须出现在视野的正前方 120 deg 范围内(通常相机视野也只有 120 deg)，也就是左右各 60deg.
    int delta = 80/angle_increment;

    int id_left = std::min( id + delta, n-1);        // 确定不是起始点和终止点
    int id_right = std::max( id - delta , 0);

    int dist_left = points.at(id_left).norm();       // length of the beam of left point (distance to laser origin)
    int dist_right = points.at(id_right).norm();

    // 逻辑别搞复杂了。
    std::vector<LineSeg> segs;
    double dist_thre = 0.05;        // distance threshold
    int skip = 3;
    int currentPt = id_right;        // laser从右往左扫描 (逆时针转)
    int nextPt = currentPt + skip;  // 每3个取一个点
    bool newSeg = true;
    LineSeg seg;
    ...
    if(debug)
    {
        ...
        bool debug_yh = true; // yanhao for not show laser image
        if(debug_yh) {
